...
export async function GET() {
  const feed = new RSS({
    title: "Yogteria products default",
    description: "Yogateria's Product default",
    generator: "RSS for Node and Next.js",
    feed_url: `${process.env.NEXT_PUBLIC_BASE_URL}/feed.xml`,
    site_url: `${process.env.NEXT_PUBLIC_BASE_URL}`,
    managingEditor: "editor@example.com (Yogateria)",
    webMaster: "editor@example.com (Yogateria)",
    copyright: `Copyright ${new Date().getFullYear().toString()}, Yogateria`,
    language: "pt-BR",
    pubDate: new Date().toUTCString(),
    ttl: 60,
  })
        ...
        return feed.item({
          ...
          custom_elements: [
            ...
            ...(product?.metadata && product?.metadata?.googleCategory
              ? [{ categories: product?.metadata?.googleCategory }]
              : []),
