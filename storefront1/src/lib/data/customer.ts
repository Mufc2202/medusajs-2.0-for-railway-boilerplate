...
export async function signup(_currentState: unknown, formData: FormData) {
  ...
  try {
    ...
    const tokenString = typeof loginToken === 'string' ? loginToken : ('location' in loginToken ? loginToken.location : '')
    await setAuthToken(tokenString)
...
export async function login(_currentState: unknown, formData: FormData) {
  ...
  try {
    await sdk.auth
      ...
      .then(async (token) => {
        const tokenString = typeof token === 'string' ? token : ('location' in token ? token.location : '')
        await setAuthToken(tokenString)
        revalidateTag("customer")
      })
  } catch (error: any) {
